...
def test_complete_ddl_with_enum_constraints():
...
def test_generate_indexes_ddl_with_foreign_keys():
    ...
    generator = TableGenerator()
    index_ddl = generator.generate_indexes_ddl(entity)
...
def test_generate_indexes_ddl_with_enum_fields():
...
def test_generate_complete_ddl_orchestration():
...
def test_tenant_specific_schema_gets_multi_tenant_fields():
...
def test_common_schema_skips_multi_tenant_fields():
...
def test_rich_types_in_complete_ddl():
...
def test_no_duplicate_comments_in_complete_ddl():
