...
def test_should_split_entity_opt_in_false():
    ...
    entity = EntityDefinition(
        ...
        fields={
            ...
            "field10": FieldDefinition(
                name="field10", type_name="text", nullable=False
            ),
            "field11": FieldDefinition(
                name="field11", type_name="text", nullable=False
            ),
            "field12": FieldDefinition(
                name="field12", type_name="text", nullable=False
            ),
...
def test_generate_node_table_ddl():
    ...
    assert (
        "fk_parent_testentity INTEGER REFERENCES test.tb_testentity_node(pk_testentity)"
        in ddl
    )
...
def test_generate_info_table_ddl():
    ...
    assert (
        "pk_testentity_info INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY" in ddl
    )
    assert "id UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE" in ddl
    assert "tenant_id UUID NOT NULL" in ddl
