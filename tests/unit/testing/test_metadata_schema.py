...
def test_entity_test_config_table_complete():
    ...
    assert (
        "default_user_id UUID DEFAULT '01232022-0000-0000-0000-000000000001'" in content
    )
...
def test_field_generator_mapping_table_complete():
    ...
    assert (
        "pk_field_gen INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY" in content
    )
    assert (
...
def test_sql_syntax_basic_validation():
    ...
    assert len([s for s in sql_statements if s.strip()]) > 5, (
...
def test_schema_file_structure():
    ...
    expected_order = [
        "tb_entity_test_config",
        "tb_field_generator_mapping",
        "tb_test_scenarios",
    ]
