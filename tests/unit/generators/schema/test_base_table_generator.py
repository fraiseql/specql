...
class TestBaseTableGeneration:
    ...
    def test_generate_base_table_with_trinity_pattern(self, generator, sample_entity):
        ...
        assert (
            "pk_contact INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY" in result
        )
    ...
    def test_generate_base_table_with_business_fields(self, generator, sample_entity):
        ...
        assert (
            "CONSTRAINT chk_tb_contact_status CHECK (status IN ('lead', 'qualified'))"
            in result
        )
    ...
    def test_generate_base_table_with_foreign_keys(self, generator, sample_entity):
        ...
        # Assert
        assert (
            "CONSTRAINT fk_contact_company FOREIGN KEY (company) REFERENCES crm.tb_company(pk_company)"
            in result
        )
    ...
    def test_generate_base_table_with_indexes(self, generator, sample_entity):
        ...
        # Assert
        assert (
            "CREATE INDEX idx_tb_contact_company ON crm.tb_contact(company)" in result
        )
...
class TestEnhancedTypeSystem:
    ...
    def test_integer_subtypes(self, generator):
        ...
        entity = Entity(
            ...
            fields={
                ...
                ),  # → BIGINT (smart default)
                "age": FieldDefinition(
                    name="age", type_name="integer:small"
                ),  # → SMALLINT (explicit)
                "count": FieldDefinition(
                    name="count", type_name="integer:int"
                ),  # → INTEGER (explicit)
                "big_num": FieldDefinition(
                    name="big_num", type_name="integer:big"
                ),  # → BIGINT (explicit)
            },
        )
        ...
        entity = Entity(
            ...
            fields={
                "price": FieldDefinition(
                    name="price", type_name="decimal:money"
                ),  # → NUMERIC(10,2)
                "latitude": FieldDefinition(
                    name="latitude", type_name="decimal:geo"
                ),  # → NUMERIC(9,6)
                "tax_rate": FieldDefinition(
                    name="tax_rate", type_name="decimal:percent"
                ),  # → NUMERIC(5,4)
                "precise": FieldDefinition(
                    name="precise", type_name="decimal:high"
                ),  # → NUMERIC(20,10)
            },
        )
    ...
    def test_text_subtypes(self, generator):
        ...
        entity = Entity(
            ...
            fields={
                "email": FieldDefinition(
                    name="email", type_name="text:short"
                ),  # → VARCHAR(255)
                "code": FieldDefinition(
                    name="code", type_name="text:tiny"
                ),  # → VARCHAR(10)
                "description": FieldDefinition(
                    name="description", type_name="text:long"
                ),  # → TEXT
    ...
    def test_context_aware_type_inference(self, generator):
        ...
        entity = Entity(
            ...
            fields={
                ...
                "organization_id": FieldDefinition(
                    name="organization_id", type_name="integer"
                ),  # → BIGINT
    ...
    def test_backward_compatibility(self, generator):
        ...
        assert (
            "count INTEGER" in result or "count BIGINT" in result
        )  # Context-dependent
