...
class TestVectorGeneration:
    ...
    def entity_with_vector(self):
        return EntityDefinition(
            name="Pattern", schema="specql_registry", features=["semantic_search"]
        )
    ...
    def test_add_embedding_column(self, generator, entity_with_vector):
        ...
        # Assert
        assert (
            "ALTER TABLE specql_registry.tb_pattern ADD COLUMN embedding vector(384)"
            in result
        )
    ...
    def test_table_view_embedding_included(self, generator, entity_with_vector):
        ...
        # Assert
        assert (
            "ALTER TABLE specql_registry.tv_pattern ADD COLUMN embedding vector(384)"
            in result
        )
    ...
    def test_similarity_search_function(self, generator, entity_with_vector):
        ...
        # Assert
        assert (
            "CREATE OR REPLACE FUNCTION specql_registry.search_pattern_by_embedding"
            in result
        )
