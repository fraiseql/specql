...
**Technical deep-dive into the framework that generates production backends** 🏗️
...
```
┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
│   YAML DSL      │ -> │   AST Parser     │ -> │  Code Generator │
│                 │    │                  │    │                 │
│ entity: User    │    │ • Validation     │    │ • PostgreSQL    │
│ fields: ...     │    │ • Type checking  │    │ • PL/pgSQL      │
│ actions: ...    │    │ • AST building   │    │ • GraphQL       │
└─────────────────┘    └──────────────────┘    └─────────────────┘
                                                        │
┌─────────────────┐    ┌──────────────────┐             │
│  PostgreSQL     │ <- │   Confiture      │ <───────────┘
│   Database      │    │   Migration      │
│                 │    │   Framework      │
└─────────────────┘    └──────────────────┘
       │
...
│   Auto-Discovery│    │                  │
│                 │    │ • Queries        │
│ • Type inference│    │ • Mutations      │
│ • Comment parsing│    │ • Subscriptions  │
└─────────────────┘    └──────────────────┘
```
...
```
db/schema/
├── 00_foundation/     # Types, extensions
├── 10_tables/         # DDL files
├── 20_helpers/        # Helper functions
└── 30_functions/      # Business functions
```
...
**Automatic Mapping:**
- PostgreSQL `TEXT` → GraphQL `String`
- PostgreSQL `INTEGER` → GraphQL `Int`
- PostgreSQL `TIMESTAMPTZ` → GraphQL `DateTime`
- Rich types → Specific scalars (`Email`, `Money`, etc.)
...
**Architecture that scales with your ambition.** 🏗️