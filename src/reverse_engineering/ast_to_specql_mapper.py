...
class ASTToSpecQLMapper:
    ...
    def _parse_declare_block(self, declare_text: str) -> List[ActionStep]:
        ...
        for match in matches:
            ...
            if (
                specql_type == var_type_lower
                and var_type_lower not in type_map.values()
            ):
            ...
            # SELECT INTO statements
                ...
                if len(parts) == 2:
                    ...
                    if len(into_parts) == 2:
                        ...
                        steps.append(
                            ActionStep(
                                type="query", sql=full_query, store_result=var_name
                            )
                        )
    ...
    def _map_cte(self, sql: str) -> ActionStep:
        ...
        match = re.search(
            r"WITH\s+(\w+)\s+AS\s*\((.*?)\)", sql, re.IGNORECASE | re.DOTALL
        )
    ...
    def _map_if(self, if_stmt: Any) -> ActionStep:
        ...
        then_steps = [
            self._map_statement(s)[0]
            for s in if_stmt.then_body
            if self._map_statement(s)
        ]
    ...
    def _map_for_query(self, for_stmt: Any) -> ActionStep:
        ...
        body_steps = [
            self._map_statement(s)[0] for s in for_stmt.body if self._map_statement(s)
        ]
    ...
    def _map_while(self, while_stmt: Any) -> ActionStep:
        ...
        body_steps = [
            self._map_statement(s)[0] for s in while_stmt.body if self._map_statement(s)
        ]
