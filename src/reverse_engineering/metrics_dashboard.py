...
def print_success_rates_table(rates: Dict[str, float], metrics: Dict[str, Dict[str, int]]):
    ...
    for parser, rate in sorted_parsers:
        m = metrics[parser]
        if m['attempts'] > 0:
            print(f"{parser:<20} {m['attempts']:>10} {m['successes']:>10} "
    ...
    print(f"{'TOTAL':<20} {total_attempts:>10} {total_successes:>10} "
...
def main():
    ...
    print("  from src.reverse_engineering.parser_coordinator import ParserCoordinator")
