...
class NLPatternGenerator:
    ...
    def generate(
        self, description: str, category: Optional[str] = None, max_retries: int = 3
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise RuntimeError(
                        f"Failed to generate pattern after {max_retries} attempts: {e}"
                    )
                continue

        raise RuntimeError(
            f"Failed to generate valid pattern after {max_retries} attempts"
        )
    ...
    def _generate_with_grok(
        ...
        if category:
            ...
            prompt = prompt.replace(
                "Category: {{ category }}", "Category: (choose appropriate)"
            )
    ...
    def _validate_pattern(self, pattern: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if not re.match(r"^[a-z][a-z0-9_]*$", name):
            errors.append(
                "Pattern name must be snake_case (lowercase, underscores only)"
            )
        ...
        has_fields = "fields" in implementation and isinstance(
            implementation["fields"], list
        )
        has_actions = "actions" in implementation and isinstance(
            implementation["actions"], list
        )

        if not has_fields and not has_actions:
            errors.append(
                "Implementation must include either 'fields' or 'actions' (or both)"
            )
                    ...
                    if not isinstance(action["steps"], list):
                        ...
                        for j, step in enumerate(action["steps"]):
                            if not isinstance(step, dict):
                                errors.append(
                                    f"Action {i} step {j} must be a JSON object"
                                )
        ...
        if has_fields:
            ...
            if not (has_pk or has_id or has_identifier):
                errors.append(
                    "Consider adding SpecQL trinity fields: pk_*, id, identifier"
                )
            ...
            if not has_audit:
                errors.append(
                    "Consider adding audit fields: created_at, updated_at, deleted_at"
                )
    ...
    def _score_confidence(
        self, pattern: Dict[str, Any], original_description: str
    ) -> float:
        """
        ...
        if isinstance(implementation, dict):
            has_fields = (
                "fields" in implementation and len(implementation["fields"]) > 0
            )
            has_actions = (
                "actions" in implementation and len(implementation["actions"]) > 0
            )
        ...
        if "fields" in implementation:
            ...
            # Naming conventions
            valid_names = sum(
                1 for name in field_names if re.match(r"^[a-z][a-z0-9_]*$", name)
            )
            if field_names:
