...
def add_missing_patterns():
    ...
    patterns_to_add = [
        ...
        (
            "foreach",
            "control_flow",
            '{"type": "foreach", "fields": ["iterator_var", "collection", "body_steps"]}',
        ...
        (
            "insert",
            "database_ops",
            '{"type": "insert", "fields": ["entity", "fields", "result_variable"]}',
            "Insert record into table",
        ),
        (
            "update",
            "database_ops",
            '{"type": "update", "fields": ["entity", "set_clause", "where_clause"]}',
            "Update records in table",
        ),
        (
            "delete",
            "database_ops",
            '{"type": "delete", "fields": ["entity", "where_clause"]}',
        ...
        (
            "duplicate_check",
            "database_ops",
            '{"type": "duplicate_check", "fields": ["entity", "check_fields", "error_message"]}',
        ...
        (
            "aggregate",
            "data_transform",
            '{"type": "aggregate", "fields": ["operation", "field", "result_variable", "table_name", "where_clause"]}',
        ...
        (
            "exception_handling",
            "control_flow",
            '{"type": "exception_handling", "fields": ["try_steps", "catch_steps", "exception_var"]}',
            "Exception handling with try/catch",
        ),
        (
            "return_early",
            "primitive",
            '{"type": "return_early", "fields": ["return_value"]}',
        ...
        (
            ...
            "Call a PostgreSQL function and store result",
        ),
        (
            "call_service",
            "primitive",
            '{"type": "call_service", "fields": ["service_name", "parameters"]}',
    ...
    implementations = [
        ...
        (
            "foreach",
            """-- ForEach: {{ iterator_var }} in {{ collection }}
    FOR {{ iterator_var }} IN
        {{ collection_query }}
    LOOP{{ loop_body }}
    END LOOP;""",
    ...
    END LOOP;""",
        ...
        (
            "for_query",
            """-- For Query: {{ iterator_var }} in ({{ query }})
    ...
    PERFORM pg_notify('{{ channel }}', {{ payload }});""",
        ...
        (
            "aggregate",
            """SELECT {{ operation }}({{ field }}) INTO {{ result_variable }}
    FROM {{ table_name }}{% if where_clause %}
    WHERE {{ where_clause }}{% endif %};""",
        ),
        (
            "json_build",
            """SELECT json_build_object({{ fields|join(', ') }}) INTO {{ result_variable }};""",
        ),
        # Exception Handling
        (
            "exception_handling",
    ...
    RETURN{% if return_value %} {{ return_value }}{% endif %};""",
        ...
        (
            "call_function",
            """SELECT {{ function_name }}({{ arguments|join(', ') }}) INTO {{ result_variable }};""",
        ),
        (
            "call_service",
