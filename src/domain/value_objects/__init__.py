...
class DomainNumber:
    ...
    def __post_init__(self):
        ...
        if not re.match(r"^[0-9a-fA-F]{2}$", self.value):
            raise ValueError(
                f"Domain number must be 2-digit hex (00-FF), got: {self.value}"
            )
...
class SubdomainNumber:
    ...
    def _validate(self):
        ...
        if len(clean_value) != 3:
            raise ValueError(
                f"Subdomain number must be 3 hex digits, got: {self.value}"
            )

        if not re.match(r"^[0-9a-fA-F]{3}$", clean_value):
            raise ValueError(
                f"Subdomain number must contain only hex digits (0-9,A-F), got: {self.value}"
            )
...
class EntitySequence:
    ...
    def _validate(self):
        ...
        if not isinstance(self.value, int):
            raise TypeError(
                f"Entity sequence must be an integer, got: {type(self.value)}"
            )

        if self.value < 0 or self.value > 9:
            raise ValueError(
                f"Entity sequence must be between 0 and 9, got: {self.value}"
            )
...
class TableCode:
    ...
    @classmethod
    def generate(
        cls, domain_num: str, subdomain_num: str, entity_seq: int, file_seq: int = 0
    ) -> "TableCode":
