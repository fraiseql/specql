...
class ConsistencyChecker:
                            ...
                            for subdomain_data in row[5]:
                                if subdomain_data:  # Skip None values
                                    ...
                                    domains[domain_num]["subdomains"][subdomain_num] = {
                                        ...
                                        "subdomain_name": subdomain_data[
                                            "subdomain_name"
                                        ],
                                        "description": subdomain_data["description"],
                                        "next_entity_sequence": subdomain_data[
            ...
            for domain_num, domain_data in data.get("domains", {}).items():
                ...
                for subdomain_num, subdomain_data in domain_data.get(
                    ...
                    domains[domain_num]["subdomains"][subdomain_num] = {
                        ...
                        "next_entity_sequence": subdomain_data.get(
    ...
    def _compare_domains(
        self, pg_domains: Dict, yaml_domains: Dict
    ) -> List[Dict[str, Any]]:
        ...
        for domain_num in all_domain_nums:
            ...
            domain_discrepancies = self._compare_domain_attributes(
                domain_num, pg_domain, yaml_domain
            )
            discrepancies.extend(domain_discrepancies)

            # Compare subdomains
            subdomain_discrepancies = self._compare_subdomains(
                domain_num, pg_domain, yaml_domain
            )
    ...
    def _compare_domain_attributes(
        self, domain_num: str, pg_domain: Dict, yaml_domain: Dict
    ) -> List[Dict]:
        """Compare domain-level attributes"""
        discrepancies = []
    ...
    def _compare_subdomain_attributes(
        self,
        domain_num: str,
        subdomain_num: str,
        pg_subdomain: Dict,
        ...
        entity_discrepancies = self._compare_entities(
            domain_num, subdomain_num, pg_entities, yaml_entities
        )
