...
class PreviewGenerator:
    ...
    def generate_preview(self, yaml_text: str, mode: str = "schema") -> PreviewResult:
        """
        ...
        try:
            ...
            return PreviewResult(
                success=True, output=output, detected_patterns=patterns
            )

        except Exception as e:
            return PreviewResult(success=False, output="", error=f"Error: {str(e)}")
    ...
    def _parse_yaml_safe(self, yaml_text: str) -> Optional[Any]:
        ...
        try:
            ...
            if entity_name:
                ...
                return EntityDefinition(name=entity_name, schema="public")
    ...
    def _generate_schema_preview(self, entity) -> str:
        ...
        try:
            ...
            # Primary key
            lines.append(
                f"    pk_{entity.name.lower()} INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,"
            )
            lines.append("    id UUID DEFAULT gen_random_uuid() NOT NULL,")

            # Fields
            for field_name, field_def in list(entity.fields.items())[
                :3
            ]:  # Limit for preview
            ...
            lines.append(
                f"CREATE INDEX idx_{entity.name.lower()}_id ON {entity.schema}.tb_{entity.name.lower()}(id);"
            )
    ...
    def _generate_fraiseql_preview(self, entity) -> str:
        ...
        try:
            ...
            # Table comment
            output.write(
                f"COMMENT ON TABLE {getattr(entity, 'schema', 'public')}.tb_{entity.entity_name.lower()} IS E'@fraiseql\n"
            )
            ...
            if hasattr(entity, "fields"):
                for field in entity.fields[:5]:  # Limit for preview
                    output.write(
                        f"COMMENT ON COLUMN {getattr(entity, 'schema', 'public')}.tb_{entity.entity_name.lower()}.{field.name} IS E'@fraiseql\n"
                    )
                    output.write(
                        f"graphql_type: {self._map_to_graphql_type(getattr(field, 'type', 'text'))}\n"
                    )
                    output.write("';\n")
