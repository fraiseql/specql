...
class PulumiGenerator:
    ...
    def _generate_network_code(
        ...
        for i in range(len(infrastructure.network.public_subnets)):
            code.append(
                f'public_rta_{i + 1} = aws.ec2.RouteTableAssociation("{infrastructure.name}-public-rta-{i + 1}",'
            )
        ...
        if infrastructure.network.enable_nat_gateway:
            ...
            for i in range(len(infrastructure.network.private_subnets)):
                code.append(
                    f'private_rta_{i + 1} = aws.ec2.RouteTableAssociation("{infrastructure.name}-private-rta-{i + 1}",'
                )
    ...
    def _generate_security_groups_code(
        self, infrastructure: UniversalInfrastructure
    ) -> List[str]:
    ...
    def _generate_load_balancer_code(
        self, infrastructure: UniversalInfrastructure
    ) -> List[str]:
