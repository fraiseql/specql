...
class GraphvizGenerator:
    ...
    def generate(
        ...
        if format not in ["svg", "png", "pdf", "dot"]:
            raise ValueError(
                f"Unsupported format: {format}. Supported: svg, png, pdf, dot"
            )
    ...
    def _add_entities_flat(self, show_fields: bool, show_trinity: bool) -> None:
    ...
    def _add_entity_node(
        self, entity: EntityNode, show_fields: bool = True, show_trinity: bool = True
    ) -> None:
        """
    ...
    def _render_diagram(self, dot_source: str, output_path: str, format: str) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to render diagram to {format}: {e}. {fallback_msg}"
            ) from e
