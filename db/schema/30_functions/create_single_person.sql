...
-- ============================================================================
-- APP WRAPPER: create_single_person
-- API Entry Point (GraphQL/REST)
-- ============================================================================
CREATE OR REPLACE FUNCTION app.create_single_person(
...
AS $$
DECLARE
    input_data app.type_create_single_person_input;
BEGIN
    -- Convert JSONB → Typed Composite
    input_data := jsonb_populate_record(
        NULL::app.type_create_single_person_input,
        input_payload
    );

    -- Delegate to core business logic
    RETURN public.create_single_person(
        auth_tenant_id,
        input_data,
        input_payload,
        auth_user_id
...
COMMENT ON FUNCTION app.create_single_person IS
'Creates a new Booker record.
Validates input and delegates to core business logic.

@fraiseql:mutation
name: createSinglePerson
input_type: app.type_create_single_person_input
success_type: CreateSinglePersonSuccess
failure_type: CreateSinglePersonError';
...
COMMENT ON FUNCTION public.create_single_person IS
'Core business logic for create single person.
...
Called by: app.create_single_person (GraphQL mutation)
